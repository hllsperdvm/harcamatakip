        ...
        function buildDemoExpenses() {
            const cats = ['Gıda', 'Araç', 'Faturalar', 'Eğlence', 'Sağlık', 'Eğitim', 'Diğer'];
            const people = ['Ali', 'Zeynep'];
            const pays = ['Nakit', 'Kredi Kartı'];
            const descs = ['Market', 'Akaryakıt', 'Elektrik', 'Kahve', 'Eczane', 'Sinema', 'Online alışveriş', 'Restoran', 'Su faturası', 'Kitap'];
            const list = [];
            for (let i = 0; i < 28; i++) {
                const cat = cats[rnd(0, cats.length - 1)];
                const dayOff = rnd(-25, 5);
                const date = ymdOffset(dayOff);
                const amount = rndAmount(45, 2800);
                const person = people[rnd(0, 1)];
                const paymentType = pays[rnd(0, 1)];
                const row = {
                    id: 'demo-exp-' + i,
                    amount: amount,
                    installmentCount: 1,
                    ...
                    date: date,
                    description: descs[rnd(0, descs.length - 1)] + ' #' + (i + 1),
                    expenseMonth: date.substring(0, 7),
                    createdAt: new Date().toISOString(),
                    isDemo: true
                };
                if (cat === 'Faturalar') {
                    row.billSubtype = ['Elektrik', 'Su', 'Doğalgaz', 'İnternet'][rnd(0, 3)];
                }
                if (cat === 'Araç' && Math.random() > 0.5) {
                    row.vehicleSubtype = 'Yakıt';
                    row.fuelKm = rnd(80, 600);
                    row.fuelLiters = rndAmount(15, 55);
                    row.fuelPricePerLt = rndAmount(40, 52);
                }
                list.push(row);
            }
            return list;
        ...
        function applyDemoPersonSelects() {
            const names = (typeof isGuest === 'function' && isGuest())
                ? ['Ali', 'Zeynep']
                : ['Bekir', 'Duygu'];
            ['person', 'filterPerson'].forEach(function(id) {
            ...
            // Kart borcu UI etiketleri
            try {
                const b = document.getElementById('bekirDebtDisplay');
                // başlık metinleri HTML sabit; demo'da borç tutarları zaten sahte
            } catch (_) {}
        }
        ...
        function loadDemoDataset() {
            ...
            notes = [
                { id: 'demo-n1', text: 'Bu bir demo notudur. Gerçek notlarınız görünmez.', createdAt: new Date().toISOString(), user: 'Misafir' },
                { id: 'demo-n2', text: 'IBAN ve özel bilgiler demo modda gösterilmez.', createdAt: new Date().toISOString(), user: 'Misafir' }
            ];
            deletedExpenses = [];
            activityLog = [
                { id: 'demo-a1', user: 'Ali', action: 'Harcama eklendi', actionType: 'Harcama', detail: 'Market · 320 TL', at: new Date().toISOString() },
                { id: 'demo-a2', user: 'Zeynep', action: 'Harcama eklendi', actionType: 'Harcama', detail: 'Kahve · 95 TL', at: new Date().toISOString() }
            ];
            categories = ['Gıda', 'Araç', 'Faturalar', 'Eğlence', 'Sağlık', 'Eğitim', 'Diğer'];
            paymentTypes = ['Nakit', 'Kredi Kartı'];
            periodConfig = { startDay: 29, endDay: 28 };
            monthlyBudgetTarget = 75000;
            bekirDebt = { amount: rndAmount(8000, 25000), paid: false, dueDate: ymdOffset(rnd(3, 12)), person: 'Ali' };
            duyguDebt = { amount: rndAmount(5000, 18000), paid: false, dueDate: ymdOffset(rnd(3, 12)), person: 'Zeynep' };
            goldHoldings = [
                { id: 'demo-g1', karat: 24, grams: rndAmount(5, 20), buyPrice: rndAmount(2800, 3200), buyDate: ymdOffset(-60), note: 'Demo' },
                { id: 'demo-g2', karat: 22, grams: rndAmount(8, 30), buyPrice: rndAmount(2500, 2900), buyDate: ymdOffset(-30), note: 'Demo' }
            ];
            familyTasks = [
                { id: 'demo-t1', text: 'Demo görev: market listesi', due: ymdOffset(2), done: false, assignee: 'Ali' },
                { id: 'demo-t2', text: 'Demo görev: fatura kontrol', due: ymdOffset(5), done: false, assignee: 'Zeynep' },
                { id: 'demo-t3', text: 'Tamamlanmış örnek', due: ymdOffset(-1), done: true, assignee: 'Ali' }
            ];
            familyCalendar = [
                { id: 'demo-c1', title: 'Demo etkinlik', date: ymdOffset(4), type: 'other' },
                { id: 'demo-c2', title: 'Örnek doğum günü', date: ymdOffset(12), type: 'birthday' }
            ];
            try {
                if (typeof ibans !== 'undefined') {
                    // IBAN gösterme — boş
                }
            } catch (_) {}
            // IBAN listesini temizle
            try { window.ibanList = []; } catch (_) {}
        }

        window.enterDemoMode = async function() {
            try {
                // Firebase oturumunu kapat + demo kilidi (gerçek veri gelmesin)
                try { sessionStorage.setItem('yuvam_demo', '1'); } catch (_) {}
                try { if (auth && auth.currentUser) await auth.signOut(); } catch (_) {}
                try { syncInitialized = true; } catch (_) {} // yeni listener açılmasın
                const profile = {
                    name: 'Misafir',
                    role: 'guest',
                    isDemo: true,
                    uid: 'demo-guest',
                    email: 'demo@yuvam.local'
                };
                // Önce bulut verisini temizle
                expenses = [];
                notes = [];
                activityLog = [];
                loadDemoDataset();
                ...
                if (label) label.textContent = 'Misafir · Demo';

                // Demo şeridi
                let bar = document.getElementById('demoModeBanner');
                if (!bar) {
                    bar = document.createElement('div');
                    bar.id = 'demoModeBanner';
                    bar.className = 'demo-mode-banner';
                    bar.innerHTML = '👀 <strong>Demo mod</strong> — Veriler rastgele örnektir · Gerçek hesap hareketleriniz gizli · Kayıt yapılmaz';
                    const root = document.getElementById('appContainer') || document.body;
                    root.insertBefore(bar, root.firstChild);
                }
                bar.classList.remove('hidden');

                try { applyDemoPersonSelects(); } catch (_) {}
                ...
                try { if (typeof updateCategorySelects === 'function') updateCategorySelects(); } catch (_) {}
                try { if (typeof renderCardDebtUI === 'function') { renderCardDebtUI('bekir'); renderCardDebtUI('duygu'); } } catch (_) {}
                try {
                    var pB = document.querySelectorAll('#bekirDebtCard p');
                    if (pB[0]) pB[0].textContent = 'Ali Kredi Kartı';
                    var pD = document.querySelectorAll('#duyguDebtCard p');
                    if (pD[0]) pD[0].textContent = 'Zeynep Kredi Kartı';
                } catch (_) {}
                ...
                try { if (typeof switchTab === 'function') switchTab('home'); } catch (_) {}

                showToast('Demo moda girdiniz — gerçek veri yok', 'info');
            } catch (err) {
                console.error(err);
                alert('Demo açılamadı: ' + (err.message || err));
            }
        };

        function guardDemoWrite(actionLabel) {
            if (!isGuest()) return false;
            if (typeof showToast === 'function') {
                showToast('Demo modda "' + (actionLabel || 'kayıt') + '" yapılmaz', 'info');
            } else {
                alert('Demo modda kayıt yapılmaz.');
            }
            return true;
        }
        ...
        window.logout = function() {
            ...
            currentUser = null;
            try { sessionStorage.removeItem('yuvam_demo'); } catch (_) {}
            try {
                const bar = document.getElementById('demoModeBanner');
                if (bar) bar.remove();
            } catch (_) {}
        ...
        function isGuest() {
            return !!(currentUser && (currentUser.role === 'guest' || currentUser.isDemo));
        }

        window.isGuest = isGuest;

        function blockCloudData() {
            try {
                if (sessionStorage.getItem('yuvam_demo') === '1') return true;
            } catch (_) {}
            return typeof isGuest === 'function' && isGuest();
        }

        async function logActivity(actionType, action, detail, userOverride) {
            if (typeof blockCloudData === 'function' && blockCloudData()) return;
            try {
                const entry = {
        ...
        function getVisibleTabs() {
            return tabsConfig.filter(t => {
                ...
                if (Array.isArray(t.visibleTo) && t.visibleTo.length) {
                    ...
                    // Misafir: Duygu ile aynı sekmeler
                    if (typeof isGuest === 'function' && isGuest()) {
                        if (t.adminOnly) return false;
                        return t.visibleTo.indexOf('Duygu') >= 0
                            || t.visibleTo.indexOf('Herkes') >= 0
                            || (t.visibleTo.indexOf('Bekir') >= 0 && t.visibleTo.indexOf('Duygu') >= 0);
                    }
                    if (!t.visibleTo.includes(currentUser.name)) return false;
                }
                return true;
            });
        }
        ...
        window.addGoldHolding = async function(e) {
            e.preventDefault();
            if (typeof guardDemoWrite === 'function' && guardDemoWrite('Altın')) return;
            if (!currentUser) {
        ...
        function resetForm() {
            ...
            try {
                ...
                if (pSel) {
                    if (typeof isGuest === 'function' && isGuest()) {
                        if (typeof applyDemoPersonSelects === 'function') applyDemoPersonSelects();
                        pSel.value = (pSel.querySelector('option[value="Ali"]') ? 'Ali' : pSel.options[0].value);
                    } else pSel.value = 'Bekir';
                }
            } catch (_) { try { document.getElementById('person').value = 'Bekir'; } catch(_2){} }
        ...
        function initRealtimeSync() {
            if (typeof isGuest === 'function' && isGuest()) {
                console.info('Demo mod: Firestore senkron atlandı');
                return;
            }
        ...
        window.handleFormSubmit = async (e) => {
            ...
            if (guardDemoWrite('Harcama')) return;
        ...
        window.deleteExpense = async (id) => {
            if (guardDemoWrite('Silme')) return;
        ...
        try {
            var _bootDemo = false;
            try { _bootDemo = sessionStorage.getItem('yuvam_demo') === '1'; } catch (_) {}
            if (auth.currentUser && !_bootDemo) {
                const loginEl = document.getElementById('errorContainer') || document.getElementById('loginScreen');
                const appEl = document.getElementById('appContainer') || document.getElementById('app');
                if (loginEl) { loginEl.classList.add('hidden'); loginEl.style.display = 'none'; }
                if (appEl) { appEl.classList.remove('hidden'); appEl.style.display = ''; }
        ...
        auth.onAuthStateChanged(async function(fbUser) {
            ...
            try {
                // Demo misafir oturumu Firebase Auth ile karışmasın
                try {
                    if (sessionStorage.getItem('yuvam_demo') === '1') return;
                } catch (_) {}
